...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:KanekiIDK\n' // Nombre
            + 'ORG:KanekiIDK;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=50582733230:+505 8273 3230\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escanea el codigo QR no te tardes weon '))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]}\n\npara usar el bot registrate con el comando ${prefix}daftar y tu nombre\n\npara ver los demas comandos usa ${prefix}help`
				...
				teks = `q insoportable q sos @${num.split('@')[0]}`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*Nivel* 0 ',
				error: {
					stick: 'no rompas las bolas, mas tarde',
					Iv: 'link invalido pelotudo'
				},
				only: {
					group: '[❗] Este comando es solo para grupos! ❌',
					ownerG: 'ese comando es solo para el owner',
					ownerB: 'ese comando es solo para Y Y=)(/"/()# BDS/UYSJH',
					admin: 'ese comando es solo para admin tontito, te cuesta',
					Badmin: 'como queres q haga eso si no soy admin boludito',
                                        daftarB: `registrate pelotudo\n\n\n\nno estas registrado en la base de datos \n\ncomando : ${prefix}daftar (tu nombre)\nejemplo : ${prefix}daftar mauri
`,
				}
			}
			...
			const ownerNumber = ["50582733230@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '50582733230@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`por boludo @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`toma @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'este es el numero de mi creador ',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                ...
                break
                   ...
                   break
				case 'wiki':
					if (args.length < 1) return reply('ESCRIBI LA PALABRA PELOTUDO')
					tels = body.slice(6)	
                                        if (!isUser) return reply(mess.only.daftarB)				
				...
				case 'ytmp3':
					...
					break
				case 'ytmp4':
					if (args.length < 1) return reply('y el url? boludo')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://st4rz.herokuapp.com/api/ytv2?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `título : ${anu.title}\n\nel video se esta mandando`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `lista de gordos *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'block':
					...
					client.sendMessage(from, `te vas bloqueado ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘hola hijos de puta〙✪══\n╠➥'+teks+'╚═〘 KanekiIDK 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘hola hijos de puta〙✪══\n╠➥'+teks+'╚═〘 KanekiIDK 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘hola hijos de puta〙✪══\n╠➥'+teks+'╚═〘 KanekiIDK 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'nos vemos ', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('y el titulo de la cancion? pelotudo pone bien')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('y la letra? gordito down')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('q buscas?')
					if (args[0].startsWith('08')) return reply('soy un bot, boludo')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('no se pudo agregar')
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`andate hijo de puta@${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'tenia unas ganas de eliminarlo', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply('falló en el momento de la conversión ${tipe} para sticker')
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('solo stickers mogolico')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('error al convertir los stickers en imagenes')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'q triste'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('solo videos pelotudo')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('no se pudo convertir el video a mp4')
						buffer = fs.readFileSync(ran)
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('pone el texto gordo de mierda')
                      if (!isUser) return reply(mess.only.daftarB)
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply('para activar: 1')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Activado')
						welkom.push(from)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('la función de bienvenida en este grupo está habilitada')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('la función de bienvenida en este grupo está deshabilitada')
					} else {
				...
				case 'logowolf':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('1 para activar los NSFW')
					if (Number(args[0]) === 1) {
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                ...
                                                buffer = await getBuffer(res.result)
                                                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'UFAASSSS si le doy '})
                                        } catch (e) {
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❌ *ERROR* ❌')
                                        }
					...
					case 'nsfwloli':
						try {
							...
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'e...'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'trap':
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'OK...'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				case 'nsfwass':
					...
					case 'nsfwahegao':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Joder, joder, me la follo?'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwarmpits':
						...
						try {
							...
							res = await fetchJson(`https://tobz-api.herokuapp.com/nsfwtrap?apikey=BotWeA`, {method: 'get'})
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'XD'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`pone el texto gordo trolo\nEjemplo: ${prefix}neonlogo thepavos`)
                                        reply(mess.wait)
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('piola')
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'fml2':
                                        ...
                                        break
                                case 'tiktok':
					if (args.length < 1) return reply('pero pone la url pelotudo')
					...
					break
				case 'ttp':
					if (args.length < 1) return reply('y el texto? hijo de puta')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('quien sos vos')
					anu = await client.chats.all()
